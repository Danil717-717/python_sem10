...
async def int_num_1(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text(f'{chr(128683)} Ошибка, допускается ввод только чисел, пожалуйста повторите ввод:')
        return float_num_one  
    except ZeroDivisionError:
        await update.message.reply_text(f"В выражении вы делите на ноль. \nИсравьте ошибку и повторите снова", reply_markup=ReplyKeyboardRemove())  
        return float_num_one        
    #except SyntaxError:
    #    #msg = bot.send_message(message.chat.id, 'Похоже, что вы написали что-то не так. \nИсравьте ошибку и повторите снова')
    #    await update.message.reply_text(f"Похоже, что вы написали что-то не так. \nИсравьте ошибку и повторите снова", reply_markup=ReplyKeyboardRemove())
    #except NameError:
    #    #msg = bot.send_message(message.chat.id, 'Переменную которую вы спрашиваете я не знаю. \nИсравьте ошибку и повторите снова')
    #    await update.message.reply_text(f"Переменную которую вы спрашиваете я не знаю. \nИсравьте ошибку и повторите снова", reply_markup=ReplyKeyboardRemove())
    #except TypeError:
    #    #msg = bot.send_message(message.chat.id, 'Мне кажется, что в выражении присутствует ошибка типов. \nИсравьте ошибку и повторите снова')
    #    await update.message.reply_text(f"Мне кажется, что в выражении присутствует ошибка типов. \nИсравьте ошибку и повторите снова", reply_markup=ReplyKeyboardRemove())
    #except ZeroDivisionError:
...
async def float_num_2(update: Update, context: ContextTypes.DEFAULT_TYPE) -> float:
    ...
    try:
        ...
        context.user_data["choice"] = y
        log.universal_logger((x, y), data_description="Ввод данных")
        await update.message.reply_text(f'Какое действие желаете выполнить с числами \    "{x}" и "{y}"?\nСумма\nВычитание\nУмножение\nДеление\n\    Целочисленное деление\nОстаток от деления\nГлавное меню\nВыход', reply_markup=markup_action)
        return action_menu
    except ValueError:
        await update.message.reply_text(f'{chr(128683)} Ошибка, допускается ввод только чисел, пожалуйста повторите ввод:')
        return float_num_two
...
def main() -> None:
    application = Application.builder().token("5614398079:AAHG5xeB3Tza3l1P0Y7vOVDPIKpK1huPglo").build()
